...
4.  **大屏图表强制**: 大屏开发中的图表组件必须使用 `BaseChart` 组件，禁止直接实例化 ECharts，以确保统一的响应式行为和生命周期管理。
5.  **大屏布局独立性**: 大屏页面（Dashboard）必须作为顶级路由独立配置，严禁嵌套在 AdminLayout 中（即不应包含侧边栏和顶栏），以保证全屏沉浸体验。
